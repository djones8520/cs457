...
//				cout << "bytes_rec: " << bytes_received << " bytes_sent: " << sentSize << endl;
				...
				uint16_t firstACKNumber;
				memcpy(&firstACKNumber, &recvBuff[0], 2);
					
				cerr << "SENDING FIRST ACK #: " << firstACKNumber << endl;
...
//				cout << endl << "end window move" << endl;
				...
				for(int k = 1; k < WINDOW_SIZE; k++){	
					if(window[k] == sequenceNumber){
						...
						uint16_t firstoooACKNumber;
						memcpy(&firstoooACKNumber, &recvBuff[0], 2);
					
						cerr << "SENDING FIRST ACK OOO #: " << firstoooACKNumber << endl;
					}
				}
			}
		...
		if (dataToWrite.count(sequenceNumber) > 0) {
			...
			uint16_t extraACKNumber;
			memcpy(&extraACKNumber, &recvBuff[0], 2);
					
			cerr << "SENDING ACK AGAIN #: " << extraACKNumber << endl;
		} else {
			// ADD PACKET TO MAP ONCE RECEIVED
			cerr << "ADDING PACKET " << sequenceNumber << " TO MAP." << endl;
			memcpy(&dataToWrite[sequenceNumber], &recvBuff[3], BYTES_TO_REC-3);
